...
const IndividualNFT=({nftname,nfttext,owner,creator,creationdate,price,index,id,priceDollar}) => {
    ...
    <div onClick={()=>{router.push(`/individualnft/${id}`)}} className={" cursor-pointer bord_grad nftwdith mt-[2.5rem] lg:mt-[3rem] transition-all duration-500 hover:-translate-y-2 h-fit"+(index==8?"hid":"")}>
        <div className="text_color h-[13rem] break-words  flex items-center text-[2.3rem] text-center px-[2rem] mt-[0.7rem] font-semibold bord-bottom font-['Inconsolata']">
             {nfttext}
        </div>
