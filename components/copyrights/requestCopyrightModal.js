...
import { NFTCopyRightSchema } from "../../schema/index"
...
function RequestCopyrightModal({setShowModal,nftName,ownerId,mutate,setLoader}) {
    ...
    const {
        ...
        initialValues,
        validationSchema: NFTCopyRightSchema,
                            ...
                            <div>
                                <div className="mx-[1.5rem] lg:ml-[1rem] xl:mx-[1.8rem] mt-[0.5rem] ">
                                    <h2 className="font-['Inconsolata'] text-[#0D1344E5']  text-[1.8rem] font-medium ml-[0.3rem] mb-[0.4rem] tracking-wider">
                                        Offered Money that is set by you is<span className="font-semibold"> {values.nftCurrency == "jagar" ? values.CopyRightPrice : ethers.utils.parseUnits(values.CopyRightPrice.toString(), 8).toString()}  Jagar </span>
                                        which is approximately   <span className="font-semibold">  {values.nftCurrency == "bnb" ? values.CopyRightPrice : ethers.utils.formatUnits(values.CopyRightPrice.toString(), 8).toString()}  BNB </span> So please
                    ...
                    <div className="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
                        ...
                        {checker == "confirm" ?
                            <button
                                className="bg-blue-500 mr-[2rem]  hover:bg-blue-700  text-white font-normal text-[1.8rem] sm:font-semibold py-3 px-[3rem] rounded-full font-['Inconsolata'] tracking-wider"
                                type="button"
                                onClick={() => copyright(nftName, values.nftCurrency == "bnb" ? ethers.utils.parseUnits(values.CopyRightPrice.toString(), 18).toString() : ethers.utils.parseUnits(values.CopyRightPrice.toString(),8).toString(), setLoader, setShowModal, setChecker, mutate, ownerId,values.CopyRightPrice)}
                            >
                                Confirm
                            </button>
                            : null
